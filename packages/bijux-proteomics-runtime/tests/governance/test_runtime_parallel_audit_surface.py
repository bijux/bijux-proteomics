...
def test_runtime_parallel_owner_is_required_for_workflow_execution() -> None:
    ...
    assert "parallel/execution.py" in execution_entry.required_modules


def test_runtime_parallel_owner_is_audited_as_execution_value() -> None:
    audit_entry = next(
        entry
        for entry in DEFAULT_RUNTIME_MODULE_AUDIT
        if entry.module_path == "parallel/execution.py"
    )
