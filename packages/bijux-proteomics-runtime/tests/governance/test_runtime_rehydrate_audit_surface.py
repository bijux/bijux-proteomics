...
def test_runtime_rehydrate_owner_is_required_for_reviewable_completed_runs() -> None:
    review_entry = next(
        entry
    ...
    assert "rehydrate/loading.py" in review_entry.required_modules


def test_runtime_rehydrate_owner_is_audited_as_execution_value() -> None:
    audit_entry = next(
        entry
        for entry in DEFAULT_RUNTIME_MODULE_AUDIT
        if entry.module_path == "rehydrate/loading.py"
    )
