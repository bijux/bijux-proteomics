...
def test_runtime_resume_owner_is_required_for_replay_and_recovery() -> None:
    replay_entry = next(
        entry
    ...
    assert "resume/execution.py" in replay_entry.required_modules


def test_runtime_resume_owner_is_audited_as_execution_value() -> None:
    audit_entry = next(
        entry
        for entry in DEFAULT_RUNTIME_MODULE_AUDIT
        if entry.module_path == "resume/execution.py"
    )
