...
def test_runtime_assurance_keeps_maxquant_search_pack_visible() -> None:
    ...
    lane = next(
        ...
        if lane.lane_id == "dda-maxquant-compatibility-pack"
    )

    assert lane.workflow_family == "dda_import"
    assert any(
        path.endswith("maxquant_evidence.tsv")
        for path in lane.repo_relative_fixture_paths
    )
    assert any(
        path.endswith("maxquant_pipeline_export.tsv")
        for path in lane.repo_relative_fixture_paths
    )
...
def test_runtime_assurance_keeps_diann_quant_pack_visible() -> None:
    ...
    lane = next(
        lane
        for lane in workflow_assurance_lanes()
        if lane.lane_id == "dia-diann-compatibility-pack"
    )

    assert lane.workflow_family == "dia_import"
    assert any(
        path.endswith("diann_report.tsv") for path in lane.repo_relative_fixture_paths
    )
    assert any(
        path.endswith("diann_pipeline_export.tsv")
        for path in lane.repo_relative_fixture_paths
    )
    assert any(
        path.endswith("diann_config.json") for path in lane.repo_relative_fixture_paths
    )
    assert all((repo_root / path).exists() for path in lane.repo_relative_fixture_paths)
