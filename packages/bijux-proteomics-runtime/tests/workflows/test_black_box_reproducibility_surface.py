...
def test_execution_mode_comparisons_keep_import_vs_raw_boundaries_explicit() -> None:
    comparisons = {row.workflow_family: row for row in build_runtime_execution_mode_comparisons()}
...
def test_replay_challenges_environment_contracts_and_refusals_stay_aligned() -> None:
    ...
    contracts = {row.workflow_family: row for row in build_runtime_environment_contracts()}
    refusals = {row.workflow_family: row for row in build_runtime_rerun_refusals()}
    stability = {row.workflow_family: row for row in build_runtime_artifact_stability_reports()}

    assert "open `packages/bijux-proteomics-core/benchmark-assets/flagship-public-packages/dda_reviewable_run/package_manifest.json`" in challenges["dda"].minimal_steps[0]
