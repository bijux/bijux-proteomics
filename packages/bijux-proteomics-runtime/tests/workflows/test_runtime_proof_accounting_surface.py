...
def test_runtime_execution_shortcut_audit_lists_only_explicit_simulation_exceptions() -> (
    None
):
...
def test_runtime_proof_map_distinguishes_raw_import_replay_and_simulation_claims() -> (
    None
):
    ...
    assert (
        claims["sequence_to_digest:review-surface"].proof_class
        is RuntimeProofClass.RAW_EXECUTION
    )
    assert (
        claims["dda_import:review-surface"].proof_class
    ...
    assert (
        claims["dia_import:review-surface"].proof_class
        is RuntimeProofClass.RAW_EXECUTION
    )
    assert (
    ...
    assert (
        claims["targeted_review:review-surface"].proof_class
        is RuntimeProofClass.RAW_EXECUTION
    )
    assert (
    ...
    assert (
        claims["dia_import:failure-replay"].proof_class
    ...
    assert (
        claims["simulated-external-engine-contract:simulation-contract"].proof_class
        is RuntimeProofClass.SIMULATION_ONLY
    )
    assert all(
...
def test_runtime_flagship_proof_gate_keeps_simulation_out_of_release_authority() -> (
    None
):
...
def test_runtime_proof_promotion_checklist_ties_missing_work_to_concrete_paths() -> (
    None
):
    ...
    assert (
        items["dda_import"].current_proof_class
        is RuntimeProofClass.IMPORT_BACKED_EXECUTION
    )
