...
`proteomics-lab` is the short alias distribution for `bijux-proteomics-lab`.

## Design constraints

- the short alias must forward to canonical lab ownership
- the package must not own independent readiness or handoff semantics
...
- Import roots: `proteomics_lab`
- Top-level families: none
- Root modules: none

## Dependency direction

This package may depend on canonical lab surfaces to preserve the short alias.
...
real owner remains `bijux-proteomics-lab`.

## Extension signals

- add code here only when preserving a documented alias surface
- keep changes forwarding-only instead of widening alias-local lab behavior
- prefer evolving the canonical lab package before touching the alias

## Misplacement signals

- if the change defines assay planning, handoff, or observed-outcome semantics,
  it belongs in `bijux-proteomics-lab`
...
- does the change preserve the short alias without inventing local lab behavior
