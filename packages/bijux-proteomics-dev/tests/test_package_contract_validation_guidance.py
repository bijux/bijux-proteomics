...
def test_package_contract_docs_keep_validation_guidance_substantive() -> None:
    failures: list[str] = []
    ...
    for package_name in _package_names():
        ...
        validation_section = _section(text, "Validation checkpoints")
        if _bullet_count(validation_section) < 3:
            failures.append(
