...
def test_maintainer_release_docs_keep_impact_guidance_substantive() -> None:
    failures: list[str] = []

    for package_name in _release_doc_packages():
        path = _package_dir(package_name) / "docs" / "maintainer" / "pypi.md"
        text = path.read_text(encoding="utf-8")
        impact_section = _section(text, "Release impact signals")
        if _bullet_count(impact_section) < 3:
            failures.append(
                f"{path.relative_to(REPO_ROOT).as_posix()}: release impact section needs at least three bullets"
            )

    index_path = REPO_ROOT / "packages" / "bijux-proteomics-dev" / "docs" / "index.md"
    index_text = index_path.read_text(encoding="utf-8")
    if _bullet_count(_section(index_text, "Release impact signals")) < 3:
        failures.append(
            f"{index_path.relative_to(REPO_ROOT).as_posix()}: release impact section needs at least three bullets"
        )

    assert not failures, "maintainer release impact guidance failed:\n" + "\n".join(
        failures
    )
