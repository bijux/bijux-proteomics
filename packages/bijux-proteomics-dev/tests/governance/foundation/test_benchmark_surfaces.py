...
def test_foundation_benchmark_surface_inventory_tracks_canonicalization_and_hashing() -> (
    None
):
    surfaces = {
        surface.benchmark_id: surface
        for surface in build_foundation_benchmark_surfaces()
    }
    ...
    assert all(
        surface.owner_focus_path.endswith("/serialization")
        for surface in surfaces.values()
    )
    assert all(
        surface.test_path.endswith(
    ...
    assert all(
        (REPO_ROOT / surface.owner_focus_path).exists() for surface in surfaces.values()
    )
