...
def test_stale_policy_entry_fails(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    _write_project(tmp_path, [])
    _write_policy(tmp_path, ["requests"])

    assert run(tmp_path) == 1
    assert (
        "Policy entries absent from root runtime dependencies"
        in capsys.readouterr().err
    )


def test_missing_policy_fails(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    _write_project(tmp_path, [])
...
def test_malformed_policy_fails(tmp_path: Path, capsys: CaptureFixture[str]) -> None:
    _write_project(tmp_path, [])
