...
def test_root_deptry_configuration_maps_all_workspace_distributions() -> None:
    deptry_config = _deptry_config()["tool"]["deptry"]
    workspace_packages = _root_pyproject()["tool"]["bijux_proteomics"]["packages"]

    module_map = deptry_config["package_module_name_map"]
...
def test_root_deptry_configuration_covers_every_workspace_package_override() -> None:
    ...
    workspace_packages = set(root_pyproject["tool"]["bijux_proteomics"]["packages"])
    package_overrides = set(_deptry_config()["tool"]["repo_deptry"]["packages"])
