...
def test_workspace_integration_patterns_match_current_package_boundaries() -> None:
    ...
    by_pattern_id = {
        validation.pattern.pattern_id: validation for validation in validations
    }

    assert all(validation.valid for validation in validations)
    assert by_pattern_id["runtime-to-core"].pattern.handoff_mode == "direct_dependency"
    assert by_pattern_id["core-to-knowledge"].pattern.handoff_mode == "artifact_handoff"
    assert (
...
def test_integration_pattern_catalog_stays_narrow_and_explicit() -> None:
    patterns = {
        pattern.pattern_id: pattern for pattern in accepted_integration_patterns()
    }
