...
def test_package_readmes_keep_consumer_impact_guidance_substantive() -> None:
    failures: list[str] = []

    for package_name in _package_names():
        path = _package_dir(package_name) / "README.md"
        text = path.read_text(encoding="utf-8")
        impact_section = _section(text, "Consumer impact signals")

        if _bullet_count(impact_section) < 3:
            failures.append(
                f"{path.relative_to(REPO_ROOT).as_posix()}: consumer impact section needs at least three bullets"
            )

    assert not failures, (
        "package README consumer impact guidance failed:\n" + "\n".join(failures)
    )
