...
from bijux_proteomics_dev.api.dependencies.module_dependency_graphs import (
...
def test_module_dependency_graphs_cover_every_workspace_package() -> None:
    ...
    assert {
        path.stem for path in MODULE_DEPENDENCY_GRAPHS_DIR.glob("*.toml")
    } == set(package_names)
...
def test_module_dependency_graph_tracks_live_internal_and_workspace_edges() -> None:
    ...
    assert any(
        ...
        for target in runtime_by_module["bijux_proteomics_runtime.runs.manager"].outgoing_internal_modules
    )
    assert any(
