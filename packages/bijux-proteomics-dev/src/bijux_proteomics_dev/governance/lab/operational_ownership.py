...
def build_lab_operational_ownership_report() -> LabOperationalOwnershipReport:
    ...
    source_owner_families = tuple(
        sorted(
            ...
            if path.is_dir() and path.name not in IGNORED_SOURCE_FAMILIES
        )
    )
    test_families = tuple(
        sorted(
            ...
            if path.is_dir() and path.name not in IGNORED_TEST_FAMILIES
        )
    )
...
def _toml_text(report: LabOperationalOwnershipReport) -> str:
    ...
    return "\n".join(
        (
            ...
            "# Regenerate with: ./.venv/bin/python -m bijux_proteomics_dev.governance.lab.operational_ownership",
            "",
            "[metrics]",
