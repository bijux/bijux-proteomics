...
from bijux_proteomics_dev.api.package_shape.package_readme_maturity import build_package_readme_maturity_report
...
from bijux_proteomics_dev.api.package_shape.package_topology_drift import build_package_topology_drift_report
from bijux_proteomics_dev.api.package_shape.package_wrapper_owner_balance import (
...
PACKAGE_REOPENED_COMPLETION_CLAIMS_PATH = (
    REPO_ROOT / "configs" / "package-governance" / "package-reopened-completion-claims.toml"
)
...
def build_package_reopened_completion_claim_report() -> PackageReopenedCompletionClaimReport:
    """Build the checked reopened completion-claim report."""

    topology = {
        entry.distribution_name: entry for entry in build_package_topology_drift_report().entries
    }
    maturity = {
        entry.distribution_name: entry for entry in build_package_readme_maturity_report().entries
    }
    root_illusions = {
    ...
    for package_name in sorted(topology):
        ...
        if topology[package_name].historical_shape_dominates_design:
            reasons.append("historical topology language still dominates current design")
        ...
        if maturity[package_name].maturity_outpaces_owner_logic:
            reasons.append("README maturity language still outruns owned logic and proof")
...
def validate_package_reopened_completion_claims(
    ...
    if total_reopened_completion_claim_count <= report.guard.max_total_reopened_completion_claim_count:
        return ()
...
def _toml_text(report: PackageReopenedCompletionClaimReport) -> str:
    lines = [
        ...
        "# Regenerate with: ./.venv/bin/python -m bijux_proteomics_dev.api.package_shape.package_reopened_completion_claims",
        "",
        "[guard]",
...
def _is_up_to_date(report: PackageReopenedCompletionClaimReport) -> bool:
    if not PACKAGE_REOPENED_COMPLETION_CLAIMS_PATH.exists():
        return False
    return PACKAGE_REOPENED_COMPLETION_CLAIMS_PATH.read_text(encoding="utf-8") == _toml_text(report)
...
def run(check: bool = False) -> int:
    ...
    if check:
        ...
        return 1
    PACKAGE_REOPENED_COMPLETION_CLAIMS_PATH.write_text(_toml_text(report), encoding="utf-8")
    print("generated package reopened completion-claim report")
    return 0


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate or validate the package reopened completion-claim report.")
