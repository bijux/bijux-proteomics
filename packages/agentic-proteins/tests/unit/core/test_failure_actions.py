...
def test_suggest_next_action_maps_failures() -> None:
    ...
    assert (
        suggest_next_action(FailureType.BIO_IMPLAUSIBLE)
        == "review_sequence_constraints"
    )
    assert (
        suggest_next_action(FailureType.HUMAN_DECISION_MISSING)
        == "provide_signed_human_decision"
    )
