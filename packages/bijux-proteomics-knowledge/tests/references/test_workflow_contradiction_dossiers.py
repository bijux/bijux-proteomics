...
def test_workflow_contradiction_dossiers_cover_each_family() -> None:
    ...
    assert {dossier.workflow_family for dossier in dossiers} == set(
        KnowledgeWorkflowFamily
    )
...
def test_workflow_contradiction_dossier_keeps_comparator_pressure_visible() -> None:
    ...
    assert any(
        "external comparator path" in scenario.comparator_position
...
def test_workflow_contradiction_dossier_links_back_to_benchmark_and_matrix_rows() -> (
    ...
    assert any(
        ref == dossier.benchmark_id for ref in dossier.scenarios[0].evidence_refs
    )
    assert any(
        ref.startswith("literature_matrix:dda:")
        for ref in dossier.scenarios[0].evidence_refs
    )
