...
def test_resolution_fixture_captures_multi_source_disagreement_patterns(
    multi_source_disagreement_bundle: EvidenceBundle,
) -> None:
    policy = ResolutionPolicy(policy_id="multi-source-policy")
    trust, resolutions = resolve_conflicts(
        multi_source_disagreement_bundle,
        policy=policy,
    )
