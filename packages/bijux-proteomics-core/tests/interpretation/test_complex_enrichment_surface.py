...
def test_build_complex_enrichment_report_evaluates_gene_and_protein_complexes() -> None:
    ...
    assert report.summary.gene_entry_count == 2
    assert report.summary.unresolved_background_count == 3

    corum_entry = next(
        entry
        for entry in report.entries
        if entry.complex_id == "CORUM:0333"
        and entry.member_kind is ComplexMemberKind.GENE
    )
    assert corum_entry.foreground_member_ids == ("MAPK1",)
    assert corum_entry.source_name == "CORUM"

    stressosome_entry = next(
        entry
        for entry in report.entries
        if entry.complex_id == "custom:stressosome"
        and entry.member_kind is ComplexMemberKind.PROTEIN
    )
    assert stressosome_entry.foreground_member_ids == ("Q99999",)
    assert any(
