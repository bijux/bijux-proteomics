...
def test_render_pathway_activity_ledgers() -> None:
    ...
    summary_tsv = render_pathway_activity_summary_tsv(report)
    matrix_tsv = render_pathway_activity_matrix_tsv(report)
    sample_tsv = render_pathway_activity_sample_score_tsv(report)
    condition_tsv = render_pathway_activity_condition_score_tsv(report)
    comparison_tsv = render_pathway_activity_condition_comparison_tsv(report)
    contribution_tsv = render_pathway_member_contribution_tsv(report)
    unresolved_tsv = render_pathway_activity_unresolved_member_tsv(report)

    assert summary_tsv.splitlines()[0].startswith("entity_level\tmeasure_kind")
    assert "pathway_id\tpathway_name\tsource_name\tsource_accession\tC1\tC2\tC3\tT1\tT2\tT3" in matrix_tsv
