...
def test_render_targeted_exports_keep_observations_and_flags_visible() -> None:
    ...
    assert "source_kind\ttransition_id\tprecursor_id\tprecursor_charge\tpeptide_sequence" in observation_tsv
    ...
    assert "target_id\tsample_id\tsource_transition_ids\tretained_transition_ids\texcluded_transition_ids\tobserved_transition_count" in sample_tsv
    ...
    assert "skyline_export\ty8\tPEPTIDEK/2\t2\tPEPTIDEK\tsample_B\t8000\t12.7\tinterference" in observation_tsv
    assert "Skyline\t2\t2\t3\t1\t0\t4\t2\t2" in summary_tsv
    assert "PEPTIDEK/2\tPEPTIDEK\tP001\ty7;y8\ty7;y8\ty8\t2\t1\t2\t273000\t136500\t12.5\t1\t1" in target_tsv
    assert "PEPTIDEK/2\tsample_B\ty7;y8\ty7\ty8\t2\t1\t1\t115000\t12.4\tinterference\t\ttrue" in sample_tsv
    assert "PEPTIDEK/2\tPEPTIDEK\tP001\t1\t1" in flagged_tsv
    assert "PEPTIDEK/2\tsample_B\ty7\t115000\t12.4\tpass" in retained_tsv
    assert "PEPTIDEK/2\tsample_B\ty8\t8000\t12.7\tinterference\tquality_filter" in excluded_tsv
