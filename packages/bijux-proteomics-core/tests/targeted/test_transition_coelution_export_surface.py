...
def test_render_targeted_transition_coelution_exports_keep_coelution_review_visible() -> (
    None
):
    ...
    assert (
        "target_id\tsample_id\texpected_transition_count\tobserved_transition_count\tcoeluting_transition_count"
        in target_tsv
    )
    assert (
        "\talignment_flagged\tcoelution_tier\treliable_transition_support\t"
        in target_tsv
    )
    assert (
    ...
    assert (
        "target_id\tsample_id\ttransition_id\tdetected\tretention_time_minutes\tanchor_transition_id"
        in transition_tsv
    )
    assert (
