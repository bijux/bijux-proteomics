...
def test_build_mechanism_cards_emits_all_required_card_classes_with_complete_evidence_fields() -> (
    ...
    assert all(
        card.confidence.value in {"high", "moderate", "low"} for card in report.cards
    )
    assert all(
        card.source_row_refs or card.derived_no_source_reason for card in report.cards
    )
    ...
    assert any(
        card.card_id.startswith("kinase-candidate-card:") for card in report.cards
    )
    assert any(card.card_id.startswith("complex-change-card:") for card in report.cards)
    assert any(
        card.card_id.startswith("compartment-signal-card:") for card in report.cards
    )
    assert any(
        card.card_id.startswith("biomarker-candidate-card:") for card in report.cards
    )
