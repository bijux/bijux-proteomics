...
def test_render_dia_dda_comparison_exports_keep_overlap_conflict_and_differential_visible() -> None:
    ...
    assert "DIA-NN\tDDA PSM\t4\t4\t2\t2\t2\t4\t4\t2\t1\t1\t1\t6\t1\t5\t2\t3\t4\t1\t1\t1\t1" in summary_tsv
    assert "P55555\tdia_only\t2\t0\t2e+06\t0" in protein_tsv
    assert "CONFLICTSEQ\tconflicting\t2\t2\t1.02e+06\t930000\tP77777\tP88888" in peptide_tsv
    ...
    assert "peptide\tCONFLICTSEQ\tconflicting\tprotein_assignment_mismatch" in conflict_tsv
    assert "protein\tP44444\tcontrol\ttreatment\ttreatment_vs_control\tconflicting\t1.1\t-1.2\t0.02\t0.03\ttrue\ttrue\topposite\tdifferential_direction_mismatch" in differential_tsv
