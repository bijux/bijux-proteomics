...
def test_dda_reviewable_package_tracks_numerical_invariants_from_real_exports() -> None:
    ...
    assert by_id["dda_reviewable_run:adapter_parity_pass_count"].observed_numeric == 2.0
    assert (
...
def test_dda_reviewable_package_turns_review_warning_into_public_demonstration() -> (
    None
):
    package = build_dda_reviewable_package()
...
def test_dda_reviewable_package_keeps_reader_facing_citations_and_runtime_artifacts() -> (
    ...
    assert any(
        path.endswith("review_packet.json") for path in package.review_artifact_paths
    )
    assert any(
