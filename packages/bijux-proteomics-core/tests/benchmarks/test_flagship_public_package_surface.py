...
def test_dda_public_benchmark_package_keeps_quality_and_lifecycle_visible() -> None:
    ...
    assert any(
        asset.path.endswith("public_benchmark_packages/dda_reviewable_run/quality_sheet.json")
        for asset in package.source_assets
    )
    assert any(
        asset.path.endswith("public_benchmark_packages/dda_reviewable_run/lifecycle.json")
        for asset in package.source_assets
    )
...
def test_dia_public_benchmark_package_promotes_library_conditioned_evidence() -> None:
    ...
    assert any(asset.path.endswith("spectronaut/spectronaut_report.tsv") for asset in package.source_assets)
    assert any(asset.path.endswith("diann/diann_pipeline_export.tsv") for asset in package.source_assets)
...
def test_lfq_public_benchmark_package_promotes_cohort_review_surface() -> None:
    ...
    assert package.runtime_availability == "no flagship runtime lane is wired yet"
...
def test_multiplex_public_benchmark_package_promotes_tmtpro_review_surface() -> None:
    ...
    assert any(asset.path.endswith("quant/multiplex_ms1_features.tsv") for asset in package.source_assets)
    assert any(asset.path.endswith("quant/multiplex.design.tsv") for asset in package.source_assets)
...
def test_targeted_public_benchmark_package_exposes_follow_up_consequences() -> None:
    ...
    assert any(asset.path.endswith("formats/targeted_benchmark_qc.tsv") for asset in package.source_assets)
...
def test_public_package_quality_sheets_name_exact_strengths_and_blockers() -> None:
    ...
    assert sheets["lfq"].runtime_state == "no flagship runtime lane is wired yet"
    ...
    assert sheets["targeted"].current_readiness == "public_package_ready_runtime_and_comparator_blocked"
