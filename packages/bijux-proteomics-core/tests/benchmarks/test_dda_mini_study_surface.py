...
def test_build_complete_dda_mini_study_bundle_requires_core_asset_roles() -> None:
    bundle = build_complete_dda_mini_study_bundle(
        ...
        asset_entries=(
            CorpusAssetEntry(
                role="spectra",
                path="inputs/run1.mzml",
                sha256="a" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="research fixture",
            ),
            CorpusAssetEntry(
                role="engine_output",
                path="inputs/engine.tsv",
                sha256="b" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="fake engine export",
            ),
            CorpusAssetEntry(
                role="fasta",
                path="inputs/db.fasta",
                sha256="c" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="truncated reference",
            ),
            CorpusAssetEntry(
                ...
                sha256="d" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="two-condition design",
            ),
            CorpusAssetEntry(
                role="identification",
                path="outputs/identification.json",
                sha256="e" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="expected ids snapshot",
            ),
            CorpusAssetEntry(
                role="protein_inference",
                path="outputs/protein.json",
                sha256="f" * 64,
                license_status=CorpusLicenseStatus.BUNDLED,
                caveat="expected protein groups",
            ),
            CorpusAssetEntry(
                role="qc",
