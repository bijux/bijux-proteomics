...
def test_evidence_graph_query_reports_render_tsv_and_json() -> None:
    ...
    assert "protein_id\tprotein_label\trelation" in render_protein_evidence_summary_tsv(
        protein
    )
    assert "peptide_id\tdepth\tnode_id" in render_peptide_support_chain_tsv(peptide)
    assert "ptm_site_id\trelation\tsource_node_id" in render_ptm_site_evidence_tsv(
        ptm_site
    )
    assert "rejected_node_id\tdepth\tnode_id" in render_rejected_evidence_path_tsv(
        rejected
    )
    assert (
        "pathway_id\tprotein_id\tprotein_label"
        in render_pathway_support_proteins_tsv(pathway)
    )
    assert "sample_id\trun_id\tqc_decision_id" in render_sample_qc_reasons_tsv(
        sample_qc
    )
...
def test_lazy_evidence_graph_queries_render_identical_export_artifacts(
    ...
    ) == render_sample_qc_reasons_tsv(query_sample_qc_reasons(graph, sample_id="S1"))
