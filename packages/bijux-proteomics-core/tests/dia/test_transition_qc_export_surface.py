...
def test_render_transition_qc_exports_keep_transition_and_weak_ledgers_visible() -> None:
    ...
    assert "transition_id\tprecursor_id\tpeptide_sequence" in transition_tsv
    assert "transition_id\tprecursor_id\tsample_id\trun_ids" in sample_tsv
    assert "transition_id\tprecursor_id\tdetected_sample_count" in weak_tsv
    assert "transition table\t2\t4\t3\t7\t5\t1" in summary_tsv
    assert "tr_y6_b\tprec_b\tACDMPEP\tP002\ty6\t512.3\t715.4\t1\t2\t6000" in (
        transition_tsv
    )
