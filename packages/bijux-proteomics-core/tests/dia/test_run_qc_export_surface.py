...
def test_render_dia_run_qc_summary_and_run_table_tsv() -> None:
    ...
    assert "DIA-NN\t3\t3\t4\t3\t4\t1\t" in summary_tsv
    assert run_table_tsv.startswith(
        "run_name\tsample_name\tprecursor_id_count\tprecursor_key_count"
    )
    assert "raw_C\tsample_C\t1\t1\t1\t1\t1\t1\t4.8451\t0.75\t0.75\ttrue" in run_table_tsv
...
def test_render_dia_run_qc_distribution_correlation_and_outlier_tsv() -> None:
    ...
    assert outlier_tsv.startswith("run_name\tsample_name\treasons")
    assert "raw_C\tsample_C\t" in outlier_tsv
    assert "precursor coverage is far below the study median" in outlier_tsv
