...
_CHEMISTRY_INIT = _PYTHON_ROOT / "chemistry" / "__init__.py"


def test_internal_modules_import_chemistry_owner_modules_directly() -> None:
    violations: list[str] = []

    for path in sorted(_PYTHON_ROOT.rglob("*.py")):
        if path in {_PACKAGE_INIT, _CHEMISTRY_INIT}:
            continue
