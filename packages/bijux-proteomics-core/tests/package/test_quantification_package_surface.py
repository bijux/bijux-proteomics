...
def test_quantification_package_exports_bootstrap_effect_stability_surface() -> None:
    table = quantification.build_label_free_intensity_table(
        (
            quantification.Ms1FeatureRecord(
                feature_id="bootstrap-pkg-001",
                sample_id="control-1",
                peptide="PEPA",
                canonical_peptide="PEPA",
                intensity=100.0,
    ...
    report = quantification.bootstrap_effect_stability(
        table,
        design_entries,
        condition_a="control",
        condition_b="case",
    ...
    rendered = quantification.render_bootstrap_effect_stability_tsv(report)
    by_entity = {entry.entity_id: entry for entry in report.entries}

    assert hasattr(quantification, "bootstrap_effect_stability")
    assert hasattr(quantification, "render_bootstrap_effect_stability_tsv")
    assert len(report.entries) == 2
    assert (
        by_entity["PSWING"].robustness_tier
        is quantification.BootstrapEffectRobustnessTier.UNSTABLE
    )
    assert rendered.startswith(
