...
"""Reviewer-facing protein grouping summaries and ledgers."""
...
class ProteinGroupingReviewEntry(JsonModel):
    """One protein group with explicit leading-protein and peptide evidence."""
...
class ProteinGroupingReviewReport(JsonModel):
    """One owned review packet over grouped protein evidence."""

    model_config = ConfigDict(extra="forbid")

    summary: ProteinGroupingSummary
    groups: tuple[ProteinGroupingReviewEntry, ...] = Field(default_factory=tuple)


def build_protein_grouping_review_report(
    records: tuple[PsmRecord, ...],
) -> ProteinGroupingReviewReport:
    """Build a direct review report over grouped protein evidence."""
    protein_groups = build_protein_groups(records)
    peptide_rollups = {
        rollup.canonical_peptide: rollup for rollup in rollup_peptide_evidence(records)
    }
    unique_counts, best_scores = _build_protein_ranking_context(records)

    groups: list[ProteinGroupingReviewEntry] = []
    ...
    return ProteinGroupingReviewReport(
        summary=ProteinGroupingSummary(
...
def render_protein_grouping_summary_tsv(report: ProteinGroupingReviewReport) -> str:
...
def render_protein_grouping_entries_tsv(report: ProteinGroupingReviewReport) -> str:
...
def _pick_leading_protein(
    ...
    *,
    ...
    if len(group.protein_refs) == 1:
        return group.protein_refs[0], "singleton_group"
