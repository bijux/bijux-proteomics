...
"""Protein and gene member resolution for pathway activity scoring."""
...
def group_pathway_records(
    pathway_records: tuple[PathwayMembershipRecord, ...],
) -> dict[str, list[PathwayMembershipRecord]]:
    """Group pathway membership records by owned pathway identifier."""
...
) -> tuple[PathwayMemberSpec, ...]:
    """Resolve each unique pathway member onto observed protein references."""

    first = records[0]
    member_specs: list[PathwayMemberSpec] = []
...
def member_label(member_kind: PathwayMemberKind, member_id: str) -> str:
    """Return the stable rendered label for one pathway member."""
