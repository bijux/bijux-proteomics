...
"""Search-engine modified peptide notation normalization."""
...
class SearchEngineModifiedPeptideDialect(StrEnum):
    """Supported search-engine peptide notation dialects."""
...
class SearchEngineModifiedPeptideReport(JsonModel):
    """Normalized view of one search-engine modified peptide string."""

    model_config = ConfigDict(extra="forbid")

    dialect: SearchEngineModifiedPeptideDialect
...
def parse_search_engine_modified_peptide(
    ...
    *,
    ...
    """Parse one engine-specific modified peptide into the owned contract."""
    resolved = (
        SearchEngineModifiedPeptideDialect(dialect)
        if isinstance(dialect, str)
        else dialect
    )
    if resolved is SearchEngineModifiedPeptideDialect.MAXQUANT:
        return _parse_maxquant_modified_peptide(notation, registry=registry)
    if resolved in {
        SearchEngineModifiedPeptideDialect.MSFRAGGER,
        SearchEngineModifiedPeptideDialect.FRAGPIPE,
        SearchEngineModifiedPeptideDialect.COMET,
    }:
...
def canonicalize_search_engine_modified_peptide(
    ...
    *,
    ...
    """Return the owned canonical notation for one engine-specific peptide."""
...
def build_search_engine_modified_peptide_report(
    ...
    *,
    ...
    """Build one reviewer-facing normalization report."""
    resolved = (
        SearchEngineModifiedPeptideDialect(dialect)
        if isinstance(dialect, str)
        else dialect
    ...
    return SearchEngineModifiedPeptideReport(
        dialect=resolved,
