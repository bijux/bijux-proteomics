        ...
        for row_number, row in enumerate(reader, start=2):
            try:
                downgrade_reasons = tuple(
                    ...
                    for reason in _split_semicolon_field(row.get("downgrade_reasons", ""))
                )
                rows.append(
                    TargetedPanelAssayInput(
                        assay_entry_id=str(row.get("assay_entry_id", "")).strip(),
...
@cli.command("targeted-panel-builder")
@click.argument(
...
@cli.command("validation-evidence-cards")
@click.argument(
    "biomarker_candidate_tsv",
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
)
...
@interpretation_group.command("pathway-activity")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
...
@interpretation_group.command("complex-activity")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
...
@interpretation_group.command("compartment-biology")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
...
@interpretation_group.command("drug-target")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
...
@interpretation_group.command("disease-phenotype")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
...
@interpretation_group.command("regulator-inference")
@click.argument(
    "input_table", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@click.argument(
