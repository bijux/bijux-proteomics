...
PACKAGE_ARTIFACT_LAYOUT_PYTHONPATH ?= $(MONOREPO_ROOT)/packages/bijux-proteomics-dev/src$(if $(PYTHONPATH),:$(PYTHONPATH))

setup: ## Prepare package artifact directories
	@PYTHONPATH="$(PACKAGE_ARTIFACT_LAYOUT_PYTHONPATH)" \
	"$(PYTHON)" -m bijux_proteomics_dev.workspace.artifact_layout package --repo-root "$(MONOREPO_ROOT)" --package-dir "$(PROJECT_DIR)"
.PHONY: setup
