...
resolve_local_rel() {
  local rel="$1"
  if [[ -d "${repo_root}/.bijux/shared" && "${rel}" == shared/* ]]; then
    printf '.bijux/%s\n' "${rel}"
    return
  fi
  printf '%s\n' "${rel}"
...
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/header.html")" "docs/overrides/partials/header.html"
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/footer.html")" "docs/overrides/partials/footer.html"
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/footer-profile-links.html")" "docs/overrides/partials/footer-profile-links.html"
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/nav.html")" "docs/overrides/partials/nav.html"
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/nav-item.html")" "docs/overrides/partials/nav-item.html"
compare_required "$(resolve_local_rel "shared/bijux-docs/partials/bijux-nav.html")" "docs/overrides/partials/bijux-nav.html"
...
for style in 00-tokens.css 01-theme.css 02-layout.css 03-header.css 04-nav.css 05-content.css 06-components.css 07-utilities.css 08-responsive.css extra.css README.md; do
  compare_required "$(resolve_local_rel "shared/bijux-docs/styles/${style}")" "docs/assets/styles/${style}"
done
...
for script in bootstrap.js detail-tabs.js nav-reveal.js nav-state.js theme-persistence.js viewport-profile.js README.md; do
  compare_required "$(resolve_local_rel "shared/bijux-docs/scripts/${script}")" "docs/assets/javascripts/shell/${script}"
done
compare_required "$(resolve_local_rel "shared/bijux-docs/scripts/nav-sync.js")" "docs/assets/javascripts/navigation-sync.js"
compare_required "$(resolve_local_rel "shared/bijux-docs/scripts/mermaid-init.js")" "docs/assets/javascripts/mermaid-init.js"
...
local_manifest_rel="$(resolve_local_rel "shared/shared-dir-sha256.txt")"
...
verify_shared_manifest_entry "${local_manifest}" "shared/bijux-gh" "local repository"
...
if [[ -f "${std_manifest}" ]]; then
  ...
  for dir_rel in shared/bijux-docs shared/bijux-makes-py shared/bijux-checks shared/bijux-gh; do
    local_rel="$(resolve_local_rel "${dir_rel}")"
    local_sha="$(directory_tree_sha256 "${repo_root}/${local_rel}")"
