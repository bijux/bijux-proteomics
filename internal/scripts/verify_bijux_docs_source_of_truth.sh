...
verify_shared_manifest_entry "${local_manifest}" "shared/bijux-gh-py" "local repository"
...
if [[ -f "${std_manifest}" ]]; then
  ...
  for dir_rel in shared/bijux-docs shared/bijux-makes-py shared/bijux-checks shared/bijux-gh-py; do
