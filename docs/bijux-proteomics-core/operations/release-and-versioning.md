...
flowchart RL
    ...
    step1["Core domain models in src/bijux_proteomics/program_spec.py"]
    step1 --> page
    step2["packages/bijux-proteomics-core/pyproject.toml"]
    step2 --> page
    step3["Package interface surface in src/bijux_proteomics/interfaces/"]
    step3 --> page
    run1["tests/ for program and lifecycle contract behavior"]
    page --> run1
    run2["cross-package and public-surface tests in tests/"]
    page --> run2
    run3["CLI and domain integration checks in tests/"]
    page --> run3
...
- release version is explicit in `packages/bijux-proteomics-core/pyproject.toml`
