...
```mermaid
flowchart LR
    edit["edit code or docs"] --> lint["lint / type / format"]
    lint --> test["run targeted tests"]
    test --> example["exercise entrypoint or example"]
    example --> review["review contract impact"]
    review --> commit["commit with evidence"]
```
