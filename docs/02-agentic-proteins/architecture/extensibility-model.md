...
```mermaid
flowchart LR
    guard1["narrow forwarding only"]
    guard2["point new work to runtime"]
    guard3["retire unused surfaces"]
    page["agentic-proteins<br/>extensibility model"]
    ext1["new module"]
...
Use `Extensibility Model` to decide whether a structural change makes `agentic-proteins` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
