...
When you need to understand a change in `agentic-proteins`, use this reading order:
...
flowchart LR
    ...
    page["agentic-proteins<br/>code navigation"]
...
Use `Code Navigation` to decide whether a structural change makes `agentic-proteins` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
