...
Changes in `agentic-proteins` should leave the package easier to explain, not
...
The foundation pages are the durable package description for `agentic-proteins`. If the package still feels blurry after this section, the boundary story is not clear enough yet.

## Visual Summary

```mermaid
flowchart LR
    driver1["keep the bridge narrow"]
    driver2["point readers to runtime"]
    driver3["retire aliases when safe"]
    pkg["agentic-proteins<br/>change principles"]
...
Use `Change Principles` to decide whether a change makes `agentic-proteins` easier or harder to defend as one distinct role in the overall system. If the work makes the package broader without making its role clearer, stop and re-check the boundary before treating the change as a local improvement.
