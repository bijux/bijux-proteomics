...
The tests for `agentic-proteins` are the executable proof of its package contract.
...
flowchart LR
    ...
    page["agentic-proteins<br/>test strategy"]
...
Use `Test Strategy` to decide whether `agentic-proteins` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
