...
flowchart LR
    ...
    page["agentic-proteins<br/>change validation"]
...
Use `Change Validation` to decide whether `agentic-proteins` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
