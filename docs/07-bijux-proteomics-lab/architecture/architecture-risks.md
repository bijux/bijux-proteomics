...
flowchart LR
    ...
    page["bijux-proteomics-lab<br/>architecture risks"]
...
Use `Architecture Risks` to decide whether a structural change makes `bijux-proteomics-lab` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
