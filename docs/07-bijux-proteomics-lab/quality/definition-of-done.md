...
A change in `bijux-proteomics-lab` is not done when code passes locally but the package contract
...
flowchart LR
    ...
    page["bijux-proteomics-lab<br/>definition of done"]
...
Use `Definition of Done` to decide whether `bijux-proteomics-lab` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
