...
flowchart LR
    risk1["stale claims"]
    risk2["hidden contradictions"]
    risk3["trust regressions"]
    page["bijux-proteomics-knowledge<br/>known limitations"]
...
Use `Known Limitations` to decide whether `bijux-proteomics-knowledge` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
