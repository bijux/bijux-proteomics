...
`bijux-proteomics-knowledge` should depend on neighboring packages only in ways that strengthen
its role as evidence state. Adjacency is acceptable when it clarifies handoff and
...
- the imports and public contracts in `packages/bijux-proteomics-knowledge`
