...
The fastest way to understand `bijux-proteomics-knowledge` is to map capabilities to the
...
```mermaid
flowchart LR
    cap1["evidence records"]
    cap2["claim transitions"]
    cap3["trust and contradiction handling"]
    pkg["bijux-proteomics-knowledge<br/>capability map"]
    use1["bijux-proteomics-intelligence"]
    use2["bijux-proteomics-lab"]
...
Use `Capability Map` to decide whether a change makes `bijux-proteomics-knowledge` easier or harder to defend as one distinct role in the overall system. If the work makes the package broader without making its role clearer, stop and re-check the boundary before treating the change as a local improvement.
