...
flowchart LR
    risk1["stale claims"]
    risk2["contradiction loss"]
    risk3["trust regressions"]
    page["bijux-proteomics-knowledge<br/>error model"]
...
Use `Error Model` to decide whether a structural change makes `bijux-proteomics-knowledge` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
