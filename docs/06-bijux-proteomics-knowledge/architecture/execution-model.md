...
`bijux-proteomics-knowledge` executes work by receiving inputs at its interfaces, coordinating policy
...
flowchart LR
    ...
    page["bijux-proteomics-knowledge<br/>execution model"]
    out1["intelligence evidence inputs"]
    out2["runtime status surfaces"]
    out3["lab outcome review"]
...
- workflow modules: src/bijux_proteomics_knowledge/model, src/bijux_proteomics_knowledge/runtime, src/bijux_proteomics_knowledge/application
...
Use `Execution Model` to decide whether a structural change makes `bijux-proteomics-knowledge` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
