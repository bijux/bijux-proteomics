...
flowchart LR
    ...
    page["bijux-proteomics-knowledge<br/>major module groups"]
    code1["src/bijux_proteomics_knowledge"]
    code2["tests"]
    code3["docs"]
...
Use `Module Map` to decide whether a structural change makes `bijux-proteomics-knowledge` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
...
Keep it aligned with the actual source directories under `packages/bijux-proteomics-knowledge`.
