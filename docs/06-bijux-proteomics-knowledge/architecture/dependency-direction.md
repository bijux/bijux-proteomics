...
```mermaid
flowchart LR
    up1["observations and artifacts"]
    up2["review decisions"]
    up3["freshness signals"]
    page["bijux-proteomics-knowledge<br/>dependency direction"]
    down1["intelligence evidence inputs"]
    down2["runtime status surfaces"]
    down3["lab outcome review"]
...
Use `Dependency Direction` to decide whether a structural change makes `bijux-proteomics-knowledge` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
