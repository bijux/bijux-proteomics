...
A change in `bijux-proteomics-intelligence` is not done when code passes locally but the package contract
...
flowchart LR
    ...
    page["bijux-proteomics-intelligence<br/>definition of done"]
...
Use `Definition of Done` to decide whether `bijux-proteomics-intelligence` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
