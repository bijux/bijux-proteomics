...
The architecture of `bijux-proteomics-intelligence` becomes readable when its major module
...
flowchart TB
    ...
    module3 --> page
    code1["src/bijux_proteomics_intelligence/model"]
    page --> code1
    code2["src/bijux_proteomics_intelligence/runtime"]
    page --> code2
    code3["src/bijux_proteomics_intelligence/application"]
    page --> code3
...
Use `Module Map` to decide whether a structural change makes `bijux-proteomics-intelligence` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
...
Keep it aligned with the actual source directories under `packages/bijux-proteomics-intelligence`.
