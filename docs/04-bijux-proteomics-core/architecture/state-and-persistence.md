...
State in `bijux-proteomics-core` should be explicit enough that a maintainer can say what is
...
```mermaid
flowchart LR
    st1["program definitions"]
    st2["lifecycle states"]
    st3["readiness decisions"]
    page["bijux-proteomics-core<br/>state and persistence"]
...
Use `State and Persistence` to decide whether a structural change makes `bijux-proteomics-core` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
