...
```mermaid
flowchart LR
    up1["foundation payloads"]
    up2["program change proposals"]
    up3["validation requirements"]
    page["bijux-proteomics-core<br/>dependency direction"]
    down1["knowledge state transitions"]
    down2["intelligence policy inputs"]
    down3["runtime execution constraints"]
...
Use `Dependency Direction` to decide whether a structural change makes `bijux-proteomics-core` easier or harder to explain in terms of modules, dependency direction, and execution flow. If the change works only because the design becomes harder to read, the safer answer is redesign rather than acceptance.
