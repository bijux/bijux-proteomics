...
The durable risks for `bijux-proteomics-core` are the ones that make the package boundary, interface contract,
...
flowchart LR
    ...
    page["bijux-proteomics-core<br/>risk register"]
...
Use `Risk Register` to decide whether `bijux-proteomics-core` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
