...
Dependency changes in `bijux-proteomics-core` should be treated as contract changes when they
...
flowchart LR
    ...
    page["bijux-proteomics-core<br/>dependency governance"]
...
Use `Dependency Governance` to decide whether `bijux-proteomics-core` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
