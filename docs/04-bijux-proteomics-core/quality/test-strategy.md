...
The tests for `bijux-proteomics-core` are the executable proof of its package contract.
...
flowchart LR
    ...
    page["bijux-proteomics-core<br/>test strategy"]
...
Use `Test Strategy` to decide whether `bijux-proteomics-core` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
