...
A change in `bijux-proteomics-core` is not done when code passes locally but the package contract
...
flowchart LR
    ...
    page["bijux-proteomics-core<br/>definition of done"]
    accept1["rules stay explicit"]
    accept2["validation stays deterministic"]
    accept3["downstream packages can rely on contracts"]
...
Use `Definition of Done` to decide whether `bijux-proteomics-core` has actually earned trust after a change. If one narrow green check hides a wider contract, risk, or validation gap, the work is not done yet.
