...
The language around `bijux-proteomics-core` should reinforce the real package
...
flowchart LR
    term1["gate"]
    term2["lifecycle state"]
    term3["readiness rule"]
    pkg["bijux-proteomics-core<br/>domain language"]
    reader1["reviewers"]
    reader2["developers"]
    reader3["maintainers"]
...
- package name: `bijux-proteomics-core`
- Python import root: `bijux_proteomics`
- owning package directory: `packages/bijux-proteomics-core`
...
Use `Domain Language` to decide whether a change makes `bijux-proteomics-core` easier or harder to defend as one distinct role in the overall system. If the work makes the package broader without making its role clearer, stop and re-check the boundary before treating the change as a local improvement.
